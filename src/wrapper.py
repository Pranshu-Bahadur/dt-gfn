...
class DTGFNClassifier:
    ...
    Handles:
      - stable target column naming
      - label encoding to contiguous 0..C-1
      - feature order freezing
    """
    ...
    def fit(self, X: Union[pd.DataFrame, np.ndarray], y: Union[pd.Series, np.ndarray]) -> "DTGFNClassifier":
        ...
        # Force a stable target name
        if isinstance(y, np.ndarray):
            y = pd.Series(y, name=_TARGET)
        ...
        feature_cols = [c for c in df_train.columns if c != _TARGET]
        ...
        # extras living on the trainer/config
